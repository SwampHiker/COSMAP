...
# define M_PI		3.14159265358979323846 // C-C C-V
...
void write_pdb(char *name, mol_type *mol)
{
  ...
  int anum;
    ...
    //    fprintf(out,"TER\nEND\n");
    fprintf(out, "TER   %5d  %-3s %3s %c%4d\nEND\n",
	    anum, " ", last_res->name, ' ', last_res->num);
    fclose(out);
}
...
// add-or-replace
      ...
      if (prevres && !res) {
	add_replace(prevres, "OXT", tmpcoords[6][0], tmpcoords[6][1], tmpcoords[6][2], FLAG_BACKBONE); //smth like that
      }
